...
interface AdminCategoriesProps {
  categories: Category[];
  onSave: (category: Partial<Category>) => Promise<boolean>;
  onDelete: (id: string) => Promise<boolean>;
}

const AdminCategories: React.FC<AdminCategoriesProps> = ({ categories, onSave, onDelete }) => {
  const [isEditing, setIsEditing] = useState<Partial<Category> | null>(null);
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-black uppercase italic font-heading">Category Registry</h1>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50 text-[10px] font-black uppercase tracking-widest text-gray-400">
                <th className="px-8 py-4">Category Designation</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {categories.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-8 py-5 font-bold text-sm uppercase">{cat.name}</td>
                  <td className="px-8 py-5 text-right space-x-2">
                    <button onClick={() => setIsEditing(cat)} className="text-gray-400 hover:text-black"><i className="fa-solid fa-pen"></i></button>
                    <button onClick={() => { if(confirm('Delete category?')) onDelete(cat.id) }} className="text-gray-400 hover:text-red-600"><i className="fa-solid fa-trash"></i></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {isEditing && (
          <div className="bg-black text-white p-8 rounded-3xl h-fit sticky top-8 animate-in slide-in-from-right-4">
            <h3 className="text-sm font-black uppercase italic mb-6 tracking-widest">Protocol: {isEditing.id ? 'Edit' : 'New'} Category</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="text-[9px] font-black uppercase text-gray-500 block mb-2">Category Name</label>
                <input 
                  type="text" 
                  autoFocus
                  required
