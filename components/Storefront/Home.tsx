  ...
  const Carousel: React.FC<{ data: Sneaker[] }> = ({ data }) => {
    ...
    return (
      <div className="relative w-full group" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
        <div ref={scrollRef} className="flex overflow-x-auto space-x-6 pb-12 pt-4 no-scrollbar scroll-smooth">
          {data.map(sneaker => <ProductCard key={sneaker.id} sneaker={sneaker} isLarge={true} />)}
        </div>
      </div>
    );
  };
  ...
  if (isLoading) {
    return (
      <div className="bg-white">
        <div className="w-full aspect-[21/9] skeleton-shimmer"></div>
        <div className="max-w-7xl mx-auto px-4 py-12 space-y-32">
          <section className="flex flex-col items-center">
            <div className="w-48 h-8 skeleton-shimmer mb-16 rounded"></div>
    ...
    <div className="bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 space-y-32">
        <section className="flex flex-col items-center">
          ...
          <button onClick={() => onNavigate('shop')} className="mt-16 px-12 py-4 border-2 border-black text-black text-[11px] font-black uppercase tracking-[0.4em] hover:bg-black hover:text-white transition-all italic shadow-lg">Shop All Products</button>
        </section>
        {sections.map((section, idx) => (
          <section key={idx} className="flex flex-col w-full">
            ...
            {section.data.length > 0 ? <Carousel data={section.data} /> : <div className="py-20 bg-gray-50 border border-dashed rounded-3xl text-center text-[10px] font-black uppercase text-gray-400 italic tracking-widest">No products found.</div>}
          </section>
        ))}
      </div>
    </div>
